...
archetypes_to_run = [7];    % if not running in batch mode, specify archetype to run [1,2,3,4,5,6,7,8]
...
micro_type = 'PCP'; % set micro source type to match chosen config files: 'PCP', 'TIRE', 'SYNTHTEX', 'PELLETS'

% Comment out 'number_MC_iterations' below to revert to reading this value in from configuration files
%number_MC_iterations = 500;

% ----------- Sensitivity analysis set-up ---------------------------------------------------------------------------

% These are only used during sensitivity runs, ignored otherwise
model_year_to_analyse = 1;  %from 1 to 'duration' (25 yrs in downloaded version)
